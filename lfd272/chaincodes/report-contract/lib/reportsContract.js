...
class ReportsContract extends Contract {
  ...
  async putRecord(ctx, id, name, price, dateString) {
    ...
    // Check if the record id is unique to avoid data rewriting
    // We are going to identify the record simply by id, so our composite
    // key should contain the id field along with an object type
    const compositeKey = stx.stub.createCompositeKey(recordObjType, [id]);
  ...
  async generateCustomReport(ctx, queryStirng) {
    return this._getResultsForQueryString(ctx, queryString);
  }
